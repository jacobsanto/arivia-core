-- Drop all problematic triggers to prevent interference
DROP TRIGGER IF EXISTS booking_financials_trigger ON guesty_bookings;
DROP FUNCTION IF EXISTS public.handle_booking_financials() CASCADE;

-- Now create the data without triggers interfering
...
-- Manually create financial data
...
INSERT INTO inventory_categories (name, description) VALUES
  ('Cleaning Supplies', 'Basic cleaning and maintenance supplies'),
  ('Linens', 'Bedding, towels, and fabric items'),
  ('Amenities', 'Guest amenities and supplies')
ON CONFLICT (name) DO NOTHING;

INSERT INTO inventory_items (name, description, unit, min_quantity, category_id) VALUES
  ('Toilet Paper', 'Standard bathroom tissue', 'rolls', 50, (SELECT id FROM inventory_categories WHERE name = 'Cleaning Supplies')),
  ('Towels', 'Bath towels for guests', 'pieces', 20, (SELECT id FROM inventory_categories WHERE name = 'Linens')),
  ('Bed Sheets', 'Queen size bed sheets', 'sets', 15, (SELECT id FROM inventory_categories WHERE name = 'Linens')),
  ('Shampoo', 'Guest bathroom shampoo', 'bottles', 10, (SELECT id FROM inventory_categories WHERE name = 'Amenities')),
  ('All-Purpose Cleaner', 'Multi-surface cleaning solution', 'bottles', 8, (SELECT id FROM inventory_categories WHERE name = 'Cleaning Supplies'))
ON CONFLICT (name) DO NOTHING;