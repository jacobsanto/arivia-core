-- Fix migration error: ensure proper UUID comparisons and (re)create policies

-- Ensure inventory_categories exists
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_inventory_categories_updated_at'
  ) THEN
...
-- Policies for inventory_usage (explicit uuid cast for safety)
