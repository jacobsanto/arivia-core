-- Drop all triggers on guesty_bookings table that might be causing issues
DROP TRIGGER IF EXISTS extract_booking_financial_data_trigger ON guesty_bookings;
DROP TRIGGER IF EXISTS booking_financial_trigger ON guesty_bookings;

-- Now create a simpler approach without problematic triggers
...
-- Manually generate financial data for the bookings
...
-- Create some housekeeping tasks manually since the triggers need to be fixed
