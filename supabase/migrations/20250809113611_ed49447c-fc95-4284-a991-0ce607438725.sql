-- Fix inventory_usage policies to avoid uuid=text compare issues when column types differ
...
CREATE TABLE IF NOT EXISTS public.inventory_usage (
  ...
  reported_by TEXT NOT NULL, -- keep flexible if existing column is TEXT
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
