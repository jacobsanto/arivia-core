  ...
  const fetchDashboardMetrics = async () => {
    try {
      ...
      const [propertiesResult, tasksResult, bookingsResult] = await Promise.allSettled([
        (supabase as any).from('guesty_listings').select('*', { count: 'exact' }).eq('is_deleted', false),
        (supabase as any).from('housekeeping_tasks').select('*', { count: 'exact' }).neq('status', 'completed'),
        (supabase as any).from('guesty_bookings').select('*', { count: 'exact' }).eq('status', 'confirmed')
      ]);
