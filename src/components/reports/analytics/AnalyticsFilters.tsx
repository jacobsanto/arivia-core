...
export const AnalyticsFilters: React.FC = () => {
  ...
  const propertyOptions: { value: PropertyFilter; label: string }[] = [
    { value: 'all', label: 'All Properties' },
    { value: 'Villa Caldera', label: 'Villa Caldera' },
    { value: 'Villa Sunset', label: 'Villa Sunset' },
    { value: 'Villa Oceana', label: 'Villa Oceana' },
    { value: 'Villa Paradiso', label: 'Villa Paradiso' },
    { value: 'Villa Azure', label: 'Villa Azure' }
  ];
    ...
    <div className="space-y-4">
      <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
        ...
        <Badge variant="outline" className="bg-muted/50 text-nowrap">
          ...
          {timeRangeFilter === 'custom' && dateRange.from && dateRange.to
            ? `${format(dateRange.from, 'MM/dd/yy')} - ${format(dateRange.to, 'MM/dd/yy')}`
            : timeRangeOptions.find(option => option.value === timeRangeFilter)?.label
          }
        </Badge>
